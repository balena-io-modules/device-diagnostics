...
Often seen on Raspberry Pi devices, these kernel messages indicate that the power supply is insufficient for the device and any peripherals that might be attached. These errors also precede seemingly erratic behaviour.
...
unclean restarts of the engine. These restarts could be caused by crash-looping. The container engine is an integral part
...
This test confirms that the hostOS properly and fully expanded the partition at boot (>80% of the total disk space has
been allocated).
...
This check confirms that the hostOS has not noted any failed boots & rollbacks.
...
These checks interrogate the engine to see if any services are restarting un-cleanly/unexpectedly or failing health checks.
...
Investigate the logs of whichever service(s) are restarting un-cleanly or failing health checks.
